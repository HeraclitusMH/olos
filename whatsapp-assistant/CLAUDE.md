...
- Recurring events: `calendar_create` takes an optional structured `recurrence` arg (`frequency` DAILY/WEEKLY/MONTHLY/YEARLY + optional `interval`/`count`/`until`/`by_day`). `app/utils/recurrence.py:build_recurrence_rule` translates it to a single RFC 5545 RRULE string passed to `GoogleCalendarService.create_event(recurrence=[...])` — a recurring series is ONE Google event, never N separate creates. `count`/`until` are mutually exclusive (count wins); date-only `until` is bumped to inclusive end-of-day then converted to UTC. Invalid recurrence raises `RecurrenceError` → friendly `ToolResult` failure (no API call). The planner sets `start`/`end` to the first occurrence only.
...
- `_process_text_message` also short-circuits the planner when normalized inbound text matches the fixed `_AGENDA_SETTINGS_TRIGGERS` set (e.g. `"agenda settings"`, `"daily agenda settings"`, `"change agenda time"`, `"customize daily agenda"`, `"agenda menu"`) — routed directly to the `daily_agenda_settings` tool, no cost-tracker increment. Normalization lowercases, collapses whitespace, and strips trailing `. , ! ?`. The planner rule for these phrases is a backup for non-exact wording.
...
- Daily agenda send strategy mirrors the reminder scheduler: try `send_text_message` first (handles the multi-line body), and on `WhatsAppAPIError` containing `131026` (outside 24-hour customer-service window) fall back to `send_template_message`. Template body parameters reject newlines/tabs and 4+ consecutive spaces (Meta error 132018), so the fallback flattens the message via `_flatten_for_template` (joins lines with ` | `). Template name/language configured via `WHATSAPP_TEMPLATE_NAME` / `WHATSAPP_TEMPLATE_LANGUAGE` (Settings defaults: `daily_agenda` / `en`); the template must have a single `{{1}}` body parameter and be approved in Meta Business Manager.
